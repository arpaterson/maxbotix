...
class rangeFinder():
	...
	def next(self):
	 ...
	 try:
		 ...
		 time.sleep( 0.2 ) # give it a chance to start ranging 
	 ...
	 if (v1[0] != -1) & (v1[1] != -1):
		 #format highbyte
		 highbyte = struct.unpack('B', chr( v1[0] ) )[0] #maxbotix sends signed 8bit int high byte
		 highbyte = highbyte & 0x7f #drop the sign bit
		 lowbyte = struct.unpack('B', chr( v1[1] ))[0] #maxbotix sends unsigned 8bit int low byte
